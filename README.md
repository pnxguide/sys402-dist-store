...
For the second step, you can write the log as `put file into [block_index] with checksum [checksum]`. The reason why we do `[checksum]` is to verify. We can verify whether the write has been done by reading the file and matching with the `[checksum]`. To revert, basically, we just need to clear the bytes.
